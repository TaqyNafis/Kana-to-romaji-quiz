...
def kana_quiz():
    while True:
        text = random.choice(kd.katakana_all)
        ...
        if len(text) == 1:
            answer = kd.katakana[text]
        elif len(text) == 2:
            romaji_comb = kd.katakana_youon_base[text[0]] + kd.katakana_youon_small[text[1]]
            answer = romaji_comb 
...
def katakana_to_romaji(text):
    ...
    while i < len(text):
        ...
        if x in kd.katakana_youon_base and i + 1 < len(text):
            if text[i+1] in kd.katakana_youon_small:
                romajinized_text += kd.katakana_youon_base[x] + kd.katakana_youon_small[text[i+1]]
                i += 2
                continue

        if x in kd.katakana:
            romajinized_text += kd.katakana[x]
        else:
            romajinized_text +=  x
...
def to_romaji_quiz ():
    while True:
        text = generate.generate_kana_text(kd.katakana_all)
        romajinized_text = katakana_to_romaji (text)
